...
<script>
  ...
  export default {
    ...
    methods:{
      ...
      handler:throttle(async function(type,disNum,cart){
        ...
        switch(type){
          ...
          case 'add':
            ...
            break
          case 'minus':
            // //  判断产品的个数大于1才能减一，否则不处理
            // if(cart.skuNum > 1){
            //   disNum = -1
            // }else{
            //   //  其实这里就是产品数量为1的时候
            //   //  其实这里前端可以拦截掉了，不用发请求占用服务器资源
            //   disNum = 0
            // }
            //  三元等式写法
      ...
      async function(cart){
        try {
          await this.$store.dispatch('reqDeletCartById',cart.skuId)
          //  如果删除成功就再次发送请求刷新展示
          this.getData()
        } catch (error) {
          alert(error.message)
      ...
      async updateChecked(cart, event){
        try {
          //  带给服务器的参数 isChecked 不是布尔值，应该是 0 | 1
          let isChecked = event.target.checked ? "1" : "0"
          //  修改状态
          this.$store.dispatch('reqUpdateCheckedById',{skuId:cart.skuId, isChecked})
          //  重新请求数据刷新状态
          this.getData()
        } catch (error) {
          //  如果失败提示
