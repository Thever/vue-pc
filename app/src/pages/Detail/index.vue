          ...
          <div class="choose">
            ...
            <div class="cartWrap">
              <div class="controls">
                ...
                <a href="javascript:" class="plus" @click.stop="skuNum++">+</a>
                <a href="javascript:" class="mins" @click.stop="skuNum>1?skuNum--:skuNum=1">-</a>
              </div>
              <div class="add">
                <!-- 以前咱们的路由调换：从A路由跳转到B路由，这里加入购物车，进行路由跳转之前，发请求把你购买的产品的信息通过请求的方式通知服务器，服务器进行相应的存储 -->
...
<script>
  ...
  export default {
    ...
    methods:{
      ...
      //  表单元素修改产品个数,这里针对input type没有指定number类型，使用默认的text
      changeSkuNum(event){
        if(event.target.value < 1 || (event.target.value*1 == NaN)){
          //  兜底，将用户输入的小于1的数转换为1,或非数字输入变成1
          this.skuNum = 1
        }else{
          //  兜底，将用户输入的小数转换成整数
      ...
      //  加入购物车的回调函数
      async addShopcar(){
        //  1.发请求----将产品加入到数据库(通知数据库)
        //  注意这里请求接口只发送了产品id和数量，并没有携带规格参数，实际项目中接口会有要求
        //  当前这里派发一个action,也向服务器发送请求
        //  同时这里也需要判断加入购物车是否成功
        //  可以将接口的返回结果返回出来获取，再根据获得的数据进行判断即可，由于接口异步返回需要时间，返回一个promise对象，那么同样适用async await 来异步获取结果即可
        try {
          await this.$store.dispatch('addOrUpdateShopCart',{skuId:this.$route.params.skuid, skuNum:this.skuNum})
          //  添加购物车成功进行路由跳转
          //  减轻服务器压力，将产品信息带给路由组件
          //  方法一：全部信息利用路由query方法传参，可行，但是地址栏比较恶心
          // this.$router.push({
          //   name:'addcartsucess',
          //   query:{
          ...
          //   }
          // })
          //  方法二：复杂的产品信息放到sessionStorage中，简单数据购买数量用query传过去
          //  sessionStorag会话结束就消失了，不会占用本地存储空间
          //  将产品信息转换成JSON字符串放到本地
          sessionStorage.setItem("addcartsucess", JSON.stringify(this.skuInfo));
          //  路由信息传参
          this.$router.push({
            name:'addcartsucess',
            query:{
