...
export default {
  ...
  data() {
    return {
      //  支付信息
      payInfo: {
        codeUrl: 'weixin://wxpay/bizpayurl?pr=NxQxHjOzz',
        orderId: 10552,
        resultCode: 'SUCCESS',
        totalFee: 3897,
      },
  ...
  computed: {
    orderId() {
      //  订单提交不成功，自己写订单号保底
      return this.$route.query.orderId || '10552'
    },
  },
  methods: {
    //  获取支付信息
    async getPayInfo() {
      let result = this.$API.reqPayInfo(this.orderId) || '10552'
      if (result.code == 200) {
        //  现有接口不能生成订单号，用旧订单号返回的data也没有对应数据
        //  为了流程自己编写对应信息
        let fakeData = {
          codeUrl: 'weixin://wxpay/bizpayurl?pr=NxQxHjOzz',
          orderId: 10552,
